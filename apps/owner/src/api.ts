 ...
 */
/**
 * App OWNER hanya untuk owner & manager. Kasir yang salah buka app ini ditolak
 * di sini (sebelum menyentuh server) supaya pesannya jelas, bukan 403 kosong.
 */
const PERAN_BOLEH = ['owner', 'manager']
 ...
 * memverifikasi klaim di dalamnya, tak pernah mengecek akun ke database. Akun
 * yang dihapus owner karena itu tetap "dianggap hidup" sampai tokennya
...
export type Pesanan = {
  ...
  /** Alasan pembatalan, ditulis kasir. null = tidak dibatalkan / tanpa alasan. */
  alasan: string | null
  /** Id kasir yang membatalkan (IAM). null = tidak dibatalkan. */
  pembatal: string | null
  /**
   * Jam mutasi status terakhir. Diambil dari `updated_at`: pembatalan dan
   * kedaluwarsa adalah mutasi terakhir sebuah pesanan, jadi nilainya == jam batal
   * (pesanan batal) atau jam kedaluwarsa (pesanan kedaluwarsa).
   */
...
export function petakanPesanan(m: Record<string, unknown>): Pesanan {
  ...
  return {
    ...
    alasan: typeof m.note === 'string' ? m.note : null,
    pembatal: typeof m.cancelled_by === 'string' ? m.cancelled_by : null,
    diubahPada: m.updated_at ? String(m.updated_at) : null,
...
/**
 * Pesanan yang DIBATALKAN outlet ini — bahan layar "Pembatalan" owner.
 *
 * Tanpa batas waktu: pembatalan jarang, dan memangkasnya di server berarti
 * menambah filter `cancelled_since` yang cuma dipakai satu layar. Layar yang
 * memutuskan seberapa jauh ke belakang ia ingin melihat.
 */
export async function ambilBatal(): Promise<Pesanan[]> {
  const data = await panggil<Record<string, unknown>[]>('/api/cashier/orders?status=cancelled')
  return Array.isArray(data) ? data.map(petakanPesanan) : []
}

/**
 * Pesanan yang KEDALUWARSA outlet ini — pelanggan tak kunjung bayar sampai
 * melewati expires_at, lalu scheduler menandainya expired (lihat
 * ExpireOrders.php). Bukan ulah kasir: tak ada pembatal, jadi tak masuk
 * kelompokkan-per-kasir — cuma ringkasan + rincian di layar yang sama.
 */
export async function ambilKedaluwarsa(): Promise<Pesanan[]> {
  const data = await panggil<Record<string, unknown>[]>('/api/cashier/orders?status=expired')
  return Array.isArray(data) ? data.map(petakanPesanan) : []
}
...
export type Laporan = {
  ...
  net: number
  /** Selisih kas shift yang ditutup di rentang ini. BUKAN bagian laba-rugi. */
  shifts: { total_variance: number; count: number; items: Shift[] }
}
...
export async function ambilLaporan(from: string, to: string): Promise<Laporan> {
  ...
  const shiftItems = (shifts.items ?? []) as Record<string, unknown>[]
  ...
  return {
    ...
    shifts: {
      total_variance: keAngka(shifts.total_variance),
      count: keAngka(shifts.count),
      items: (Array.isArray(shiftItems) ? shiftItems : []).map(petakanShift),
    },
  }
}
...
export async function hapusKategori(id: string): Promise<void> {
  await mintaJson(`/api/categories/${encodeURIComponent(id)}`, { method: 'DELETE' }, CATALOG)
}
...
export async function hapusProduk(id: string): Promise<void> {
  await mintaJson(`/api/products/${encodeURIComponent(id)}`, { method: 'DELETE' }, CATALOG)
}
...
export async function hapusStaf(id: string): Promise<void> {
  await mintaJson(`/api/staff/${encodeURIComponent(id)}`, { method: 'DELETE' }, IAM)
}
...
// ---------------------------------------------------------------------------
// Promo (Catalog) — kelola promo owner; kasir tak menyentuh bagian ini.
// ---------------------------------------------------------------------------
...
export type Promo = {
  ...
  template: string
  status: string
  statusEfektif: string
  persen: number | null
  nominal: number | null
  minSubtotal: number
  maksDiskon: number | null
  produk: { productId: string; qty: number }[]
}

export type TemplatePromo = {
  key: string
  label: string
  description: string
}

export function petakanPromo(m: Record<string, unknown>): Promo {
  return {
    id: String(m.id ?? ''),
    nama: typeof m.name === 'string' ? m.name : '',
    template: typeof m.template === 'string' ? m.template : '',
    status: typeof m.status === 'string' ? m.status : '',
    // Server menghitung "scheduled"/"expired" dari jadwal, bukan status mentah.
    statusEfektif: typeof m.effective_status === 'string' ? m.effective_status : '',
    persen: m.percentage == null ? null : keAngka(m.percentage),
    nominal: m.amount == null ? null : keAngka(m.amount),
    minSubtotal: keAngka(m.min_subtotal) || 0,
    maksDiskon: m.max_discount == null ? null : keAngka(m.max_discount),
    produk: Array.isArray(m.products)
      ? m.products.map((p: Record<string, unknown>) => ({
          productId: String(p.product_id ?? ''),
          qty: Number(p.required_qty ?? 1) || 1,
        }))
      : [],
  }
}

export async function ambilPromo(): Promise<Promo[]> {
  const data = await panggil<Record<string, unknown>[]>('/api/promotions', undefined, CATALOG)

  return (Array.isArray(data) ? data : []).map(petakanPromo)
}

export async function ambilTemplatePromo(): Promise<TemplatePromo[]> {
  const data = await panggil<Record<string, unknown>[]>('/api/promotion-templates', undefined, CATALOG)
...
export type PayloadPromo = {
  name: string
  template: string
  percentage?: number
  amount?: number
  min_subtotal?: number
  products?: { product_id: string; required_qty: number }[]
}

export async function buatPromo(payload: PayloadPromo): Promise<void> {
  await mintaJson(
    '/api/promotions',
    {
      method: 'POST',
...
export async function aktifkanPromo(id: string): Promise<void> {
  await mintaJson(`/api/promotions/${encodeURIComponent(id)}/activate`, { method: 'POST' }, CATALOG)
}

export async function pausePromo(id: string): Promise<void> {
  await mintaJson(`/api/promotions/${encodeURIComponent(id)}/pause`, { method: 'POST' }, CATALOG)
}

export async function hapusPromo(id: string): Promise<void> {
  await mintaJson(`/api/promotions/${encodeURIComponent(id)}`, { method: 'DELETE' }, CATALOG)
}
